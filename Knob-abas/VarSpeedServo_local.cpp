...
VarSpeedServo_local::VarSpeedServo_local()
{
...
uint8_t VarSpeedServo_local::attach(int pin)
{
  return this->attach(pin, MIN_PULSE_WIDTH, MAX_PULSE_WIDTH);
}

uint8_t VarSpeedServo_local::attach(int pin, int min, int max)
{
...
void VarSpeedServo_local::detach()
{
...
void VarSpeedServo_local::write(int value)
{
...
void VarSpeedServo_local::writeMicroseconds(int value)
{
...
*/
void VarSpeedServo_local::write(int value, uint8_t speed) {
...
void VarSpeedServo_local::write(int value, uint8_t speed, bool wait) {
  write(value, speed);
...
void VarSpeedServo_local::stop() {
  write(read());
}

void VarSpeedServo_local::slowmove(int value, uint8_t speed) {
...
int VarSpeedServo_local::read() // return the value as degrees
{
  return  map( this->readMicroseconds()+1, SERVO_MIN(), SERVO_MAX(), 0, 180);
}

int VarSpeedServo_local::readMicroseconds()
{
...
bool VarSpeedServo_local::attached()
{
  return servos[this->servoIndex].Pin.isActive ;
}

uint8_t VarSpeedServo_local::sequencePlay(servoSequencePoint sequenceIn[], uint8_t numPositions, bool loop, uint8_t startPos) {
...
uint8_t VarSpeedServo_local::sequencePlay(servoSequencePoint sequenceIn[], uint8_t numPositions) {
  return sequencePlay(sequenceIn, numPositions, true, 0);
}

void VarSpeedServo_local::sequenceStop() {
  write(read());
  this->curSeqPosition = CURRENT_SEQUENCE_STOP;
}

// to be used only with "write(value, speed)"
void VarSpeedServo_local::wait() {
  byte channel = this->servoIndex;
...
bool VarSpeedServo_local::isMoving() {
  byte channel = this->servoIndex;
  int value = servos[channel].value;
...
/*
	To do
int VarSpeedServo_local::targetPosition() {
	byte channel = this->servoIndex;
	return map( servos[channel].target+1, SERVO_MIN(), SERVO_MAX(), 0, 180);
}

int VarSpeedServo_local::targetPositionMicroseconds() {
	byte channel = this->servoIndex;
